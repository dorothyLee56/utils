...
export const getEndOfCurrentDay = (): number => {
  ...
  const day: number = new Date(nowTime).getDate();
...
const getEndOfCurrentWeek = (): number => {
  ...
  const lastDayOfWeek: Date = new Date(now.getTime() + (6 - diff) * 24 * 60 * 60 * 1000); // 本周最后一天（周日）
 ...
 */
function padLeft (value: string | number, num: number, pad?: any) {
  let result = value + '';
